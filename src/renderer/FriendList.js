...
import { ChevronRight, ChevronUp, Search } from "tabler-icons-react";

import { Input, NavLink, Collapse, Indicator, Avatar } from "@mantine/core";
...
export default React.memo(function FriendList() {
  ...
  return (
    <>
      ...
      <FriendListGroup
        friends={favorites.filter((f) => fuzzysearch(query, f.name))}
        header="Favorites - 10/32"
      />

      <FriendListGroup
        friends={friends.filter((f) => fuzzysearch(query, f.name))}
        header="Friends - 10/32"
      />
    </>
  );
});
